...
def tree_from_sexp(sexp):
    ...
    """
    return nltk.Tree.fromstring(sexp, remove_empty_top_bracketing=True)
...
def train(training):
    ...
    for key, productions in lhs_map.items():
        ...
        for production in productions:
            weights[production] = max(1, int(counts[production]/float(total)*100))
    regexp_hash = {}
    for key in keys:
...
def print_weights(lhs_map, weights):
    ...
    for key in keys:
        ...
        for production in productions:
            rhs_symbols = [str(nt) for nt in production.rhs()]
            print "%d\t%s\t%s" %(weights[production], production.lhs(), ' '.join(rhs_symbols))
...
if __name__ == "__main__":
    ...
    if options.input is None:
        ...
        replace_weights(regexp_hash, weights, options.input, options.output, options.normalize)   
